...
codex_cli_step() {
  ...
  if ! component_is_expected codex_cli; then
    report_set_component_outcome "codex_cli" "$COMPONENT_OUTCOME_DISABLED"
    step_result_skipped "A configuração do Codex CLI foi desativada por configuração."
    return 0
  fi

  if component_detect codex_cli; then
    report_set_component_outcome "codex_cli" "$COMPONENT_OUTCOME_REUSED"
    step_result_skipped "O Codex CLI já estava configurado."
    return 0
  fi

  if component_apply codex_cli; then
    report_set_component_outcome "codex_cli" "$COMPONENT_OUTCOME_CHANGED"
    step_result_success "O Codex CLI foi configurado."
    return 0
  fi

  report_set_component_outcome "codex_cli" "$COMPONENT_OUTCOME_FAILED"
